...
% To save figures
plotdir = '/Users/robertjones/Desktop/F23/599/hw/hw2/code2upload/plots_p2';
if ~exist(plotdir,'dir'), mkdir(plotdir); end

%%  Load brain maps data

load('brain_maps.mat');
dims = size(T1map);

% Display brain maps images
f = figure('position',[175 509 1051 265],'color','w','InvertHardcopy','off');
subplot(131); 
imshow(M0map,[]); title('M0 map'); set(gca,'FontSize',16);
cbar=colorbar;
cbar.Title.String= 'M0';
subplot(132);
imshow(T1map,[]); title('T1 map'); set(gca,'FontSize',16);
cbar=colorbar;
cbar.Title.String= 'T1 (ms)';
subplot(133);
imshow(T2map,[]); title('T2 map'); set(gca,'FontSize',16);
cbar=colorbar;
cbar.Title.String= 'T2 (ms)';
...
%% Part c
% reduce echo spacing (5ms to 2ms) for etl=128 to improve image contrast!
